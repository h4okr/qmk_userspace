    ...
    if ((mods & MOD_MASK_SHIFT) != 0  // Shift is held.
      ...
      for (int i = 0; i < (int)custom_shift_keys_count(); ++i) {
        ...
        if (keycode == custom_shift_key->keycode) {
          ...
          // h4okr’s addition : call process_record_user in case of custom keycodes
          if (registered_keycode >= SAFE_RANGE) {
            del_weak_mods(MOD_MASK_SHIFT);
#ifndef NO_ACTION_ONESHOT
            del_oneshot_mods(MOD_MASK_SHIFT);
#endif  // NO_ACTION_ONESHOT
            unregister_mods(MOD_MASK_SHIFT);
            process_record_user(registered_keycode, record);
            set_mods(mods);   
            // end of h4okr’s addition   
